...
exports.fashionSystemPrompt = exports.buildWardrobeContext = exports.detectTrend = exports.TREND_SCORES = void 0;
...
function buildOutfitSnippets(items, maxOutfits = 5) {
    ...
    const top = candidates
        ...
        .slice(0, maxOutfits);
    if (top.length === 0)
        return "No complete outfit combinations available yet.";
    return top
        .map((o, i) => {
...
function detectDominantAesthetic(items) {
...
async function buildWardrobeContext(userId) {
    ...
    // Wardrobe gaps
    const gaps = [];
    if (!bySection.has("Footwear"))
        gaps.push("no footwear");
    if (!bySection.has("Accessories"))
        gaps.push("no accessories");
    if ((bySection.get("Tops") ?? 0) === 0 && (bySection.get("Dresses") ?? 0) === 0) {
        gaps.push("no tops or dresses");
    }
    if ((bySection.get("Bottoms") ?? 0) === 0)
        gaps.push("no bottoms");
    return `
╔══════════════════════════════════════════════════════════════╗
...
exports.buildWardrobeContext = buildWardrobeContext;
// ─── Master fashion stylist system prompt ─────────────────────────────────────
function fashionSystemPrompt(wardrobeContext) {
...
exports.fashionSystemPrompt = fashionSystemPrompt;
