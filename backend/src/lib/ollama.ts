...
export async function isOllamaRunning(): Promise<boolean> {
  ...
  try {
    await axios.get(`${OLLAMA_BASE}/api/tags`, { timeout: 2_000 });
...
export async function ollamaVision(
  ...
  try {
    ...
    const msg: string = e?.response?.data?.error ?? e?.message ?? "Ollama error";

    // Timeout = model is too slow on this hardware — enter cooldown
    if (msg.includes("timeout") || msg.includes("ETIMEDOUT")) {
      ollamaTimeoutUntil = Date.now() + TIMEOUT_COOLDOWN_MS;
      console.warn(
        `[ollama] Vision timed out — entering ${TIMEOUT_COOLDOWN_MS / 60000}-min cooldown. ` +
...
export async function ollamaChat(
  ...
  try {
    ...
    const msg: string = e?.response?.data?.error ?? e?.message ?? "Ollama chat error";
    if (msg.includes("ECONNREFUSED")) { ollamaReachable = false; return null; }
